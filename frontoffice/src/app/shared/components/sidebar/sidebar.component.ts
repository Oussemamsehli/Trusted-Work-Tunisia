...
export class SidebarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userService.getMyProfile().subscribe({
      next: (data: UserProfileResponse) => {
        this.trustLevel = Number((data as any).trustLevel ?? 1);
        this.currentUserId = Number((data as any).id ?? (data as any).userId ?? this.currentUserId ?? 0);

        this.displayName = this.resolveNameFromProfile(data) || this.displayName;
        this.roleLabel = this.resolveRoleLabel((data as any).role ?? this.currentUser?.role);

        const discoverGroup = this.navGroups.find(group => group.label === 'Découvrir');
        const publicProfileItem = discoverGroup?.children.find(child => child.label === 'Mon profil public');

        if (publicProfileItem && this.currentUserId > 0) {
          publicProfileItem.route = `/app/profile/public/${this.currentUserId}`;
  ...
  get trustPercent(): number {
    const normalized = Math.max(0, Math.min(5, this.trustLevel));
  ...
  trackByChild(_: number, child: NavChild): string {
  ...
  private resolveNameFromAuthUser(user: AuthUser | null): string {
    ...
    const found = possibleValues.find(value => !!value && String(value).trim().length > 0);
  ...
  private resolveNameFromProfile(profile: UserProfileResponse | null | undefined): string {
    ...
    const match = candidates.find(value => !!value && String(value).trim().length > 0);
